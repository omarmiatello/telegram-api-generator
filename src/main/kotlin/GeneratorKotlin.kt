...
fun List<DocSection>.toKotlinModels(useKotlinXSerialization: Boolean) = buildString {
    ...
    appendLine("data class TelegramResponse<T>(")
    appendLine("    val ok: Boolean,")
    appendLine("    val result: T? = null,")
    if (useKotlinXSerialization) appendLine("    @SerialName(\"error_code\")")
    appendLine("    val errorCode: Int? = null,")
    appendLine("    val description: String? = null,")
    appendLine(")")
    appendLine()
...
fun List<DocSection>.toKotlinMethods() = buildString {
    ...
    appendLine("import io.ktor.client.request.HttpRequestBuilder")
    ...
    appendLine("import io.ktor.http.isSuccess")
    appendLine("import kotlinx.serialization.KSerializer")
    ...
    appendLine("import kotlinx.serialization.json.Json")
    appendLine()
    appendLine("open class TelegramBotClientException(message: String, errorCode: Int) : RuntimeException(message)")
    appendLine()
    ...
    appendLine("""    private val baseFilePath = "${"$"}apiUrl/file/bot${"$"}apiKey"""")
    appendLine("    private val json = Json {")
    appendLine("        ignoreUnknownKeys = true")
    appendLine("        prettyPrint = true")
    appendLine("        encodeDefaults = false")
    appendLine("    }")
    appendLine()
    appendLine("    private suspend fun <T> telegramGet(path: String, serializer: KSerializer<T>): Result<T> =")
    appendLine("        runCatching {")
    appendLine("            val responseString: String = httpClient.get(path) {")
    appendLine("                this.requestConfigurer()")
    appendLine("            }.body()")
    appendLine("            val result: TelegramResponse<T> =")
    appendLine("                json.decodeFromString(TelegramResponse.serializer(serializer), responseString)")
    appendLine("            if (!result.ok || result.result == null) {")
    appendLine("                throw TelegramBotClientException(result.description ?: \"empty\", result.errorCode ?: 0)")
    appendLine("            }")
    appendLine("            result.result ?: throw IllegalStateException(\"no result\")")
    appendLine("        }")
    appendLine()
    appendLine("    private suspend fun <T> telegramPost(path: String, body: String, serializer: KSerializer<T>): Result<T> =")
    appendLine("        runCatching {")
    ...
    appendLine("        }")
    appendLine()
    appendLine("    suspend fun downloadFile(filePath: String): Result<TelegramResponse<ByteArray>> = runCatching {")
    appendLine("        val response = httpClient.get(\"\$baseFilePath/\$filePath\") {")
    appendLine("            requestConfigurer()")
    appendLine("        }")
    appendLine("        if (response.status.isSuccess()) {")
    appendLine("            TelegramResponse(ok = true, result = response.body<ByteArray>())")
    appendLine("        } else {")
    appendLine("            val errorResponse: TelegramResponse<String> =")
    appendLine("                json.decodeFromString(TelegramResponse.serializer(String.serializer()), response.body<String>())")
    appendLine("            TelegramResponse(ok = false, description = errorResponse.description, errorCode = errorResponse.errorCode)")
    appendLine("        }")
    appendLine("    }")
